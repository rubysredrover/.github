...
- **Spatial Memory (SAM)** — continuously builds a map of objects and their locations in the environment. Using segmentation (SAM), the robot identifies and tracks objects as it moves through space, storing this in a local, queryable spatial memory. This allows VLA to reference “what is where” instantly, instead of reasoning over raw perception.
...
Training runs overnight on the edge appliance while the child sleeps, using otherwise idle compute. The system operates in two modes: real-time during the day, learning at night.
